...
class User(db.Model, UserMixin):
    ...
    force_pw_change: Mapped[bool] = mapped_column(db.Boolean, default=False)
            ...
            birthdays = [
                {
                    ...
                    "birthday": f"{u[0].date_of_birth.day}. {month_mappings[u[0].date_of_birth.month]}",
                }
                for u in user_birthdays
            ]
    ...
    def last_login(self):
        result = db.session.execute(
            db.select(UserLogin.user_agent, UserLogin.login_time)
...
class UserPreferences(db.Model):
    ...
    order_users_by: Mapped[str | None] = mapped_column(db.String(20))
    show_claims: Mapped[bool] = mapped_column(db.Boolean, default=True)
    ...
    @classmethod
    def create(
        cls, user_id: int, order_users_by: str | None = None, show_claims: bool = True
    ) -> "UserPreferences":
        prefs = cls()
...
class PasswordResetToken(db.Model):
    ...
    def __repr__(self) -> str:
        return f"<PasswordResetToken token_id={self.token_id!r} user_id={self.user_id}>"

    @classmethod
    def create(
        cls, token_id: str, token: str, user_id: int, expires_at: datetime
    ) -> "PasswordResetToken":
        entry = cls()
...
class UserLogin(db.Model):
    ...
    credential: Mapped[int | None] = mapped_column(
        db.Integer, ForeignKey("webauthn_credential.entry_id"), index=True
    )  # , ondelete="SET NULL"))
